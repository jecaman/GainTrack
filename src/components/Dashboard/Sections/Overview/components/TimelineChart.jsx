...
const TimelineChart = ({ portfolioData, theme, hiddenAssets = new Set(), excludedOperations = new Set(), disabledOps = new Set(), showApplyPopup, setShowApplyPopup, startDate: externalStartDate, endDate: externalEndDate, buttonStartDate, buttonEndDate, setStartDate: setExternalStartDate, setEndDate: setExternalEndDate, onTimelineApplyToAll, showTimelinePopup, showTimelineClickPopup, sidebarOpen, timelineUnfreezeTooltipRef, filterSelectedPreset, isInPointClickMode, setIsInPointClickMode, onFilterReset, isApplyingFromTimeline, viewMode: viewModeProp = 'both', onViewModeChange, showTotalInvested: showTotalInvestedProp = false, onShowTotalInvestedChange, periodMode: periodModeProp = 'day', onPeriodModeChange }) => {
  ...
  const getTooltipFontSize = (contentLength, baseSize = 22) => {
    ...
    // Tamaño mínimo legible moderado — en mobile el tooltip también puede envolver línea
    // (ver renderTooltipContent), así que no necesita ser tan grande como para caber en una sola línea
    const minFontSize = window.innerWidth < 768 ? 11 : 20;
  ...
  const renderTooltipContent = (dataIndex = null, processedData = null) => {
    ...
    return `
      <div class="custom-tooltip" style="
        ...
        padding: 0;
        margin: 0;
        min-width: ${isMobile ? '0' : 'max-content'};
  ...
  return (
    <div className="timeline-chart-container" style={{
      ...
      <div style={{
        ...
        <div style={{
          position: 'relative',
          marginBottom: isMobile ? '0.5rem' : '8rem',
          transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)'
        }}>
          {/* Contenedor principal para controles — en mobile pasa a flujo normal y envuelve línea,
              en vez de quedarse posicionado absolutamente fuera del ancho de la pantalla */}
          <div style={{
          position: isMobile ? 'static' : 'absolute',
          top: '0',
          left: isMobile ? 'auto' : '60px',
          right: isMobile ? 'auto' : '60px', // Márgenes aumentados a 60px en ambos lados
          display: 'flex',
          ...
          {/* Controles izquierda — contiene el pill de vista, la leyenda Y los botones de fecha;
              necesita su propio flex-wrap en mobile o los tres se apretujan en una sola línea. */}
          <div style={{
            display: 'flex',
            flexWrap: isMobile ? 'wrap' : 'nowrap',
            alignItems: 'center',
          ...
          <div style={{ display: 'flex', flexWrap: isMobile ? 'wrap' : 'nowrap', alignItems: 'center', gap: isMobile ? '0.75rem' : '1.5rem' }}>
            {viewMode === 'both' && (
              <div style={{ 
                display: 'flex', 
                alignItems: 'center', 
          ...
          {/* Botones de fechas a la derecha — en flujo normal en mobile, para que envuelva
              línea con el resto de controles en vez de quedar fuera de la pantalla */}
          <div style={{
            position: isMobile ? 'static' : 'absolute',
            top: '6px',
            right: isMobile ? 'auto' : (sidebarOpen ? '-30px' : '20px'), // Otro pelin más a la derecha
            // width:100% + flexBasis:100% fuerza que este bloque siempre ocupe su propia línea
            // dentro del contenedor con flex-wrap, en vez de compartir línea con la leyenda y cortarse.
      ...
      <div style={{ height: isMobile ? '1rem' : '4rem' }}></div>
      ...
      <div style={{
        ...
        <div id="tooltip-area" style={{
          ...
          position: 'relative',
          padding: '10px 8px',
          paddingLeft: isMobile ? '0.75rem' : '60px', // Alineado con el margen izquierdo del gráfico
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'flex-start',
          minWidth: 0,
      ...
      <div style={{
        ...
        marginRight: isMobile ? '0.75rem' : (sidebarOpen ? '-20px' : '60px'), // Margen negativo para extender más allá del borde
        position: 'relative',
      }}>
        <Line 
          ref={chartRef} 
        ...
        {/* Botones de filtro rápido - debajo del gráfico; en mobile pasa a flujo normal y envuelve línea */}
        <div style={{
          position: isMobile ? 'static' : 'absolute',
        ...
        {((!isUsingDefaultRange || isZoomed || isTooltipFrozen)) && (
          <div
            ...
            style={{
              ...
              top: isMobile ? 'auto' : ((sidebarOpen && showTotalInvested && viewMode !== 'balance') ? '-5px' : '-65px'),
